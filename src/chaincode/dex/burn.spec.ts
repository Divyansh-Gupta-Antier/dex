...
describe("Remove Liquidity Test", () => {
  ...
  it("Should throw SlippageToleranceExceededError if returned amount0  received is below amount0min", async () => {
    ...
    expect(res).toEqual(
      GalaChainResponse.Error(
        new SlippageToleranceExceededError(
          "Slippage tolerance exceeded: expected minimums (amount0 ≥ 0.04, amount1 ≥ 0), but received (amount0 = 0.03905535002371904128, amount1 = 68.5329680198566)"
        )
      )
    );
  });
  ...
  it("Should throw SlippageToleranceExceededError if amount1  received is below amount1min", async () => {
    ...
    expect(res).toEqual(
      GalaChainResponse.Error(
        new SlippageToleranceExceededError(
          "Slippage tolerance exceeded: expected minimums (amount0 ≥ 0, amount1 ≥ 70), but received (amount0 = 0.03905535002371904128, amount1 = 68.5329680198566)"
        )
      )
    );
  });
